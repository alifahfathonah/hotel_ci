...
class Model_transaksi_ruangan extends CI_Model
{
	...
	function post()
	{
		...
		if (!empty($this->input->post('lama_sewa')) && !empty($this->input->post('lama_waktu'))) {

			$lama_sewa = date('Y-m-d H:i:s', strtotime($tanggal_sewa.' +'. $hari .' days'));
			$lama_waktu = date('Y-m-d H:i:s', strtotime($lama_sewa.' +'. $waktu .' hours'));

			$total_bayar = ($hari * $waktu) * $this->input->post('harga');
